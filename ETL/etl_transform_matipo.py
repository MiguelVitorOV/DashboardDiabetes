...
def transform_matipo():
    input_dir = 'data/extracted'
    output_dir = 'data/processed/matipo'
    ...
    for file_path in files:
        ...
        print(f"Transformando dados Matipó {year}...")
        
        df_all = pd.read_csv(file_path, sep=';')
        
        # Filtrar por Matipó (co_municipio_residencia = 314070)
        df = df_all[df_all['co_municipio_residencia'] == 314070].copy()
        
        # Limpeza de idade
        if not df.empty and df['nu_idade'].dtype == object:
        ...
        output_file = os.path.join(output_dir, f'dados_diabetes_processados_Matipo_{year}.csv')
